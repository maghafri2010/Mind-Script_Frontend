            ...
            <div className="flex flex-wrap justify-center gap-10">
                {labels.map((label, i) => (
                    <div key={keys[i]} className="flex flex-wrap gap-15  w-[470px]  ">
                        ...
                        {isEditing ? (
                            <input className={"bg-amber-50"} className="inputStyle edit" value={editProfile[keys[i]]} onChange={ e => setEditProfile(prev => ({...prev, [keys[i]]: e.target.value}))}/>
                        ) : (
                            <span className="inputStyle pt-[9px] edit">{profile[keys[i]]}</span>
                        )}
