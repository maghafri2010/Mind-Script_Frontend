...
const New = ({setIndex, index}) => {
    ...
    return(
        <section className='relative flex'>
            ...
            ${status ? "w-[160px] h-[400px]" : "w-0 h-0 "}`}>
                {buttonList.map((button, i) => (
                    <button key={i} onClick={() => {setIndex(i);  openNew();}} className={`box  rounded-4xl p-2 flex items-center gap-2 `}>
                        {button.name}
                        <span alt={button.name} className="w-10 h-10 bg-white" >{button.img}</span>
                    </button>
                ))} 
            </div>
            ...
            <div 
  onClick={openNew} 
  className="fixed bottom-10 right-10 w-16 h-16 border-2 box border-white p-2 overflow-hidden rounded-[50%] flex items-center justify-center z-50 cursor-pointer"
>
