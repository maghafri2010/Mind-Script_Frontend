...
const New = ({setIndex, index}) => {
    ...
    return(
        <section className='relative flex'>
            ...
            <div 
            className=
            {`z-50 fixed items-end gap-4 bottom-20 right-10 flex flex-col transition-all duration-300 justify-center  overflow-hidden ease-in-out 
            ${status ? "w-[160px] h-[400px]" : "w-0 h-0 "}`}>
                {buttonList.map((button, i) => (
                    <button key={i} onClick={() => {setIndex(i);  openNew();}} className={`box  rounded-4xl p-2 flex items-center justify-center gap-2 `}>
                        {button.name}
                        <span alt={button.name} className="w-[50px] h-[50px] bg-white" >{button.img}</span>
                    </button>
                ))} 
            </div>
