...
const keys = ["Username:", "Date Joined", "First Name:", "Last Name:", "Email:", "Phone:"];
            ...
            <div className="flex flex-wrap justify-center gap-10">
                {keys.map((key, i) => (
                    <div className="flex flex-wrap gap-15  w-[470px]  ">
                        <p className="text-2xl font-bold w-36 ">{key}</p>
                    
                        {isEditing ? (
                            <input className="inputStyle edit" value={profile[keys[i]]} onChange={e => setProfile({[keys[i]]: e.target.value})}/>
                        ) : (
                            <span className="inputStyle edit">{profile[keys[i]]}</span>
                        )}
                        <img onClick={edit} className="iconEdit box"  src={editButton} alt="editButton" />
                    </div>
                ))}
